                                ...
                                <div id="items-container">
                                    ...
                                    <div class="item-row row">
                                        <div class="form-group col-md-4">
                                            ...
                                            <input type="hidden" name="items[{{ $index }}][barang_id]" value="{{ $item->barang->id }}">
                                        </div>
                                        <div class="form-group col-md-2">
                                            <label for="qty">Quantity</label>
                                            <input type="number" class="form-control qty-input" name="items[{{ $index }}][qty]" value="{{ $item->qty }}" required min="1">
                                        </div>
                                        <div class="form-group col-md-2">
                                            <label for="verify_{{ $index }}">Verifikasi Qty Barang</label>
                                            <div class="custom-control custom-switch">
                                                <input type="checkbox" class="custom-control-input toggle-verify" id="verify_{{ $index }}">
                                                <label class="custom-control-label" for="verify_{{ $index }}"></label>
                                            </div>
                                        </div>
                                    </div>
                                    @endforeach
