                ...
                <tr>
                    ...
                    <td class="text-right">
                        <p>
                            ...
                            {{ \Carbon\Carbon::parse($endDate)->translatedFormat('d M Y') }} <br>
                        </p>
                    </td>
                </tr>
            </table>
            ...
            <table class="table">
                <thead>
                    <tr>
                        ...
                        @endif
                        <th class="text-left"><strong>{{ $type == 'barang_keluar' ? 'Invoice' : 'PO' }} No.</strong>
                        </th>
                        <th class="text-left"><strong>{{ $type == 'barang_keluar' ? 'Invoice' : 'PO' }} Date</strong>
                        </th>
                        <th class="text-left"><strong>Barang</strong></th>
                        @if ($type == 'barang_broken')
                ...
                <tbody>
                    @if ($datatransaksi->isEmpty())
                        <tr>
                            <td {{ $type == 'barang_keluar' || $type == 'barang_broken' ? 'colspan=5' : 'colspan=4' }}
                                class="no-data text-center">
                                Data transaksi tidak ditemukan untuk periode yang dipilih.</td>
                        </tr>
                    @else
                        @foreach ($datatransaksi as $detailitem)
                            ...
                            <tr class="invoice-header">
                                ...
                                <td colspan="{{ $type == 'barang_broken' ? '3' : '2' }}">Dibuat Oleh:
                                    {{ $detailitem->user->name }}</td>
                            </tr>
                            <tr>
                                @if ($type == 'barang_keluar')
                                    <td></td>
                            ...
                            <tr>
                                <td {{ $type == 'barang_keluar' || $type == 'barang_broken' ? 'colspan=4' : 'colspan=3' }}
                                    class="no-line text-right"><strong>Total</strong></td>
                                <td class="no-line text-center">
                                    <strong>{{ $detailitem->details->sum(fn($item) => $item->qty) }}</strong>
                                </td>
                            </tr>
                        @endforeach
                    @endif
                ...
                <tfoot>
                    <tr>
                        <td {{ $type == 'barang_keluar' || $type == 'barang_broken' ? 'colspan=4' : 'colspan=3' }}
                            class="text-right"><strong>Sub
                                Total</strong></td>
                        <td class="text-center">
                            <strong>{{ $datatransaksi->flatMap(function ($item) {return $item->details;})->sum('qty') }}</strong>
                        </td>
                    </tr>
                </tfoot>
            </table>
